...
- `AnalysisDimensionsTraitBinNumber.min` and `AnalysisDimensionsTraitBinDate.min` are now
  optional. When omitted, the bin is open-ended below its `max`, counting all values below
  that range — the mirror of an omitted `max` (open-ended above `min`).
