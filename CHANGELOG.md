...
- `faraday_pixel` dataset `options.type`: a Faraday-hosted source for reidentifying anonymous website visitors. It takes no additional options, and an account may have at most one.

## [0.15.0] - 2026-06-24

### Changed

- **Breaking:** `AnalysisDimensionsTraitBinNumber.min` and `AnalysisDimensionsTraitBinDate.min`
  are now optional — consumers can no longer assume `min` is always present. When omitted, the
  bin is open-ended below its `max`, counting all values below that range (the mirror of an
  omitted `max`, which is open-ended above `min`).

## [0.14.25] - 2026-06-10

### Added

- `Attribute.type` is now writable on create and update (previously read-only).
- `Attribute.statistical_type`: optional `TraitStatisticalType` describing the nature of the attribute's data.
- `Attribute.unit`: optional string giving the units a numeric attribute is stored in.
- `Dataset.output_to_authority`: optional `{ name, date }` that tags a dataset's contents as a dated assertion from a named authority. Available only for vendor accounts.
- `Attribute.sources`: optional effective-dated source configuration nested by authority, source column, and inclusive effective-from date. Each entry sets precision, quality, optional derivations/effective_until, and an optional transform (cast, selector, or decode/recode).

## [0.14.24] - 2026-06-11

### Deprecated

- `GET /persona_sets/{persona_set_id}/analysis/flow` (`getPersonaSetAnalysisFlow`) is now deprecated and no longer maintained.
...
- `impute_payload` field on `TargetModesHashed`, `TargetModesIdentified`, and `TargetModesReferenced` representations — array of payload elements to include imputed values for. Defaults to `[]` (off). Pass `["attributes"]` to include attribute values imputed from cohabitants and neighbors; otherwise the target's payload only includes attribute values from rows matched at person-level precision. `TargetModesAggregated` does not support this field.
...
- Optional `limit` field on `AccountUsageTimePeriod` and `AccountUsagePredictiveData` — exposes the customer's yearly contractual limit. Populated only on the rolled-up `/v1/accounts/current/usage/all` endpoint (the per-account endpoints omit it because the contractual limit applies to the parent + sub-accounts total). Omitted when no limit is on record.
...
- Add read-only `type` field on `/attributes` endpoint - the data type of the attribute's value property, derived from stream event_stream_properties.

## [0.14.12] - 2026-04-10

### Added

- Add `breaks` on GET `/attributes` endpoint - for continuous data types, list of reasonable cutoff values.

## [0.14.11] - 2026-04-09

### Added

- `binary`, `count`, `interval`, `ratio` values for `TraitStatisticalType` enum.
- `fig/vehicles`, `fig/civic`, `fig/engagement` values for `StreamPropertyCategory` enum.

## [0.14.10] - 2026-04-01

### Added

- `predictors.max_observation_date` optional date field on outcomes and recommenders for release pinning — caps FIG attribute data used during modeling to a specific date.
- `payload.max_attribute_observation_date` optional date field on scopes for release pinning — caps FIG attribute data included in scope output to a specific date.
- `modeling_field_max_observation_date` optional date field on persona sets for release pinning — caps FIG attribute data used in modeling fields to a specific date.

## [0.14.9] - 2026-04-01

### Added

- `sum` and `count_not_null` aggregation methods for `AttributeAggregationMethod`.
...
- `identity_graph` field on accounts, indicating which feature store the account uses for retrieving metadata such as attributes.
...
- GET `/accounts/current/usage` - returns usage metrics for the current account, including batch deployments, lookup API calls, and predictive data usage.
- GET `/accounts/current/usage/all` - returns aggregated usage metrics across the current account and all sub-accounts.
- `contract_started_at` field on accounts.
...
- Made `aggregation` and `default_selector_by_purpose` optional on `/attributes` endpoints (POST, PUT, GET) to match the database schema where these columns are nullable.
...
- Changed allowed values for `category` in `/streams` endpoints. Should match the values allowed in `/traits` endpoints.

## [0.11.4] - 2025-10-20

### Added

- Added `interpretation` to `GET /persona_sets/{persona_set_id}`

## [0.11.3] - 2025-10-20

### Deprecated

- In `dataset.identity_sets`, `address_line_1` and `address_line_2` are deprecated. Use `house_number_and_street` array format instead to compose address lines.
...
- Added `salient` to `AnalysisDimensionsTrait` to show whether the trait distinguishes the persona from the rest of the persona set.

## [0.11.0] - 2025-09-05

### Breaking

- No longer support zip or tar uploads due to security issues

## [0.10.62] - 2025-09-04

### Changed

- Allow `/` in SFTP dataset prefix
